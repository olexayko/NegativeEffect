...
func shoot():
	...
	var slash = load("res://abilities/effects/apathy_effect.tscn").instance()
	slash.get_node("HitBox").condition_type = character.condition_type
	if !character.get_node("ProjectileSpawn").rotation_degrees:
		...
		slash.global_position = character.get_node("ProjectileSpawn/Right").global_position
